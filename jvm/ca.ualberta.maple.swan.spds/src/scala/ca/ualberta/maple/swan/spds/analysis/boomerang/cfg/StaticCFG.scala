...
package ca.ualberta.maple.swan.spds.analysis.boomerang.cfg

import ca.ualberta.maple.swan.spds.analysis.boomerang.scene.Statement

class StaticCFG extends ObservableCFG {

  override def addPredsOfListener(l: PredecessorListener): Unit = {
    l.curr.method.getCFG.getPredsOf(l.curr).foreach(s => l.handlePredecessor(s))
  }

  override def addSuccsOfListener(l: SuccessorListener): Unit = {
    l.curr.method.getCFG.getSuccsOf(l.curr).foreach(s => l.handleSuccessor(s))
  }
