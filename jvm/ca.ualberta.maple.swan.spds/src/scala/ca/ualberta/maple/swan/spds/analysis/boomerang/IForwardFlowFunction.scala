...
trait IForwardFlowFunction {
  ...
  def callFlow(callSite: Statement, factAtCallSite: Val, callee: Method): mutable.HashSet[Val]

  def normalFlow(query: ForwardQuery, edge: ControlFlowGraph.Edge, fact: Val): mutable.HashSet[State]

  def callToReturnFlow(query: ForwardQuery, edge: ControlFlowGraph.Edge, fact: Val): mutable.HashSet[State]

  def setSolver(solver: ForwardBoomerangSolver[_],
