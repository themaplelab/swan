...
import ca.ualberta.maple.swan.spds.analysis.boomerang.scene.{ControlFlowGraph, Statement, StaticFieldLoadStatement, StaticFieldVal, Val}
...
class FlowSensitiveStaticFieldStrategy[W <: Weight] extends StaticFieldStrategy[W] {
  ...
  override def handleBackward(curr: ControlFlowGraph.Edge[Statement, Statement], leftOp: Val, staticField: StaticFieldVal, out: mutable.HashSet[State]): Unit = {
    out.add(new Node(curr, curr.target.asInstanceOf[StaticFieldLoadStatement].getStaticField))
  }
}
