...
trait IBackwardFlowFunction {
  ...
  def callFlow(callSite: CallSiteStatement, fact: Val, callee: Method, calleeSp: Statement): mutable.HashSet[Val]

  def normalFlow(edge: Edge[Statement, Statement], fact: Val): mutable.HashSet[State]

  def callToReturnFlow(edge: Edge[Statement, Statement], fact: Val): mutable.HashSet[State]

  def setSolver(solver: BackwardBoomerangSolver[_ <: Weight],
