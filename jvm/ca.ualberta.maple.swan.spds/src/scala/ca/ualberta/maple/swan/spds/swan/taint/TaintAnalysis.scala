...
package ca.ualberta.maple.swan.spds.swan.taint
...
import ca.ualberta.maple.swan.spds.analysis.boomerang.{BackwardQuery, Boomerang, BoomerangOptions, DefaultBoomerangOptions, ForwardQuery}
import ca.ualberta.maple.swan.spds.analysis.boomerang.scene.Val.AllocVal
import ca.ualberta.maple.swan.spds.analysis.boomerang.scene.{ControlFlowGraph, DataFlowScope, Method, Statement, Val}
...
import ca.ualberta.maple.swan.spds.swan.{SWANCallGraph, SWANInvokeExpr, SWANMethod, SWANStatement}
...
class TaintAnalysis(val spec: TaintSpecification, val opts: TaintAnalysisOptions) {
  ...
  class ForwardMethodTaintQuery(val edge: ControlFlowGraph.Edge[Statement, Statement],
                                variable: AllocVal, val source: Method,
                                val sinks: mutable.HashSet[SWANMethod]) extends ForwardQuery(edge, variable)

  class BackwardMethodTaintQuery(val edge: ControlFlowGraph.Edge[Statement, Statement],
                                 variable: Val, val sources: mutable.HashSet[SWANMethod],
      ...
      case AnalysisType.Backward => {
        ...
        seedsArray.foreach(i => {
          ...
          seeds.forEach(query => {
            ...
            /*
            queryResults.getAllocationSites.foreach(x => {
  ...
  private def generateForwardSeeds(cg: SWANCallGraph,
    ...
    spec.sources.foreach(src => {
      ...
      cg.edgesInto(source).foreach(edge => {
        val stmt = edge.callSite
        seeds.add(new ForwardMethodTaintQuery(
          new ControlFlowGraph.Edge[Statement, Statement](stmt, stmt),
          new AllocVal(stmt.lhs, stmt, stmt.lhs),
          source, sinks))
      })
  ...
  private def generateBackwardSeeds(cg: SWANCallGraph,
    ...
    spec.sinks.foreach(s => {
      ...
      cg.edgesInto(sink).foreach(cgEdge => {
        val cfEdge = new ControlFlowGraph.Edge[Statement, Statement](
          cgEdge.callSite,
          cgEdge.callSite.method.getCFG.getSuccsOf(cgEdge.callSite).head)
        val apply = cgEdge.callSite.asInstanceOf[SWANStatement.CallSite]
        apply.getInvokeExpr.getArgs.foreach(v => {
      ...
      sink.getParameterLocals.foreach(param => {
        sink.getCFG.getStartPoints.foreach(start => {
          val edge = new ControlFlowGraph.Edge[Statement, Statement](
            start,
            sink.getCFG.getSuccsOf(start).head)
    ...
    spec.sinks.foreach(sink => {
      ...
      cg.edgesInto(sinkMethod).foreach(edge => {
        results.cellSet().forEach(cell => {
          if (cell.getRowKey.start == edge.callSite && edge.callSite.uses(cell.getColumnKey)) {
            var skip = false
            if (sink._2.args.nonEmpty) {
              val invokeIdx = edge.callSite.getInvokeExpr.asInstanceOf[SWANInvokeExpr].getIndexOf(cell.getColumnKey)
            ...
            if (!skip) {
              val srcStmt =  query.edge.start.asInstanceOf[SWANStatement]
              val sinkStmt = cell.getRowKey.start.asInstanceOf[SWANStatement]
              val nodes = new ArrayBuffer[(CanInstructionDef, Option[Position])]
              nodes.append((srcStmt.getDelegate, srcStmt.getPosition))
              nodes.append((sinkStmt.getDelegate, sinkStmt.getPosition))
