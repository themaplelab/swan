...
abstract class PointOfIndirection {

  protected val actualBaseAllocations: mutable.HashSet[ForwardQuery] = mutable.HashSet.empty
  protected val flowAllocations: mutable.HashSet[Query] = mutable.HashSet.empty
  ...
  def addBaseAllocation(baseAllocation: ForwardQuery): Unit = {
    if (actualBaseAllocations.add(baseAllocation)) {
      flowAllocations.foreach(flowAllocation => execute(baseAllocation, flowAllocation))
    }
  }

  def addFlowAllocation(flowAllocation: Query): Unit = {
    if (flowAllocations.add(flowAllocation)) {
      actualBaseAllocations.foreach(baseAllocation => execute(baseAllocation, flowAllocation))
    }
  }
}
