...
public class RawAstTranslator extends SILInstructionVisitor<CAstNode, SILInstructionContext> {
    ...
    public CAstEntity translate(File file, CAstNode n) {
        ...
        for (CAstNode function : mappedEntities.keySet()) {
            ...
            if (Inliner.shouldInlineFunction(C.parent.getName(), C)) {
                continue;
            }
            ...
            for (CAstNode block : function.getChild(4).getChildren()) {
                if (Inliner.shouldInlineBlock(blockNo, C) && blockNo > 0) {
                    continue;
                }
                C.clearInstructions();
    ...
    protected AbstractCodeEntity generateNewEntity(String name, ArrayList<RawValue> arguments, RawValue returnVal) {
        String returnType = returnVal != null ? returnVal.Type : "void";
        CAstSourcePositionMap.Position functionPosition = null; // TODO
        ArrayList<String> argumentNames = new ArrayList<>();
        ArrayList<String> argumentTypes = new ArrayList<>();
        ArrayList<CAstSourcePositionMap.Position> argumentPositions = new ArrayList<>();
        for (RawValue arg : arguments) {
            String argName = arg.Name;
            String argType = arg.Type;
            CAstSourcePositionMap.Position argPosition = null; // TODO
            argumentNames.add(argName);
            argumentTypes.add(argType);
            argumentPositions.add(argPosition);
        }
        argumentNames.add(0, "self");
        argumentTypes.add(0, "self");
        argumentPositions.add(0, null);
        AbstractCodeEntity newEntity = new FunctionEntity(name, returnType, argumentTypes, argumentNames, functionPosition, argumentPositions);
        newEntity.setAst(Ast.makeNode(CAstNode.EMPTY));
        return newEntity;
    }
    ...
    protected CAstNode visitFunctionRef(CAstNode N, SILInstructionContext C) {
        ...
        if (!BuiltInFunctionSummaries.isBuiltIn(FuncName) && !BuiltInFunctionSummaries.isSummarized(FuncName) && C.allEntities.containsKey(FuncName)) {
            ...
            SILFunctionRef.ArtificialFunctionRef FuncRef =
                    new SILFunctionRef.ArtificialFunctionRef(result.Name, result.Type, C, FuncName);
            C.valueTable.addValue(FuncRef);
        }
        return null;
    }
    ...
    protected CAstNode visitApply(CAstNode N, SILInstructionContext C) {
        ...
        SILValue TempFuncRef = C.valueTable.getValue(FuncRefName);
        CAstNode Source;
        // Check if the func ref exists in the value table since operator overloading is possible.
        boolean exists;
        if (TempFuncRef instanceof SILFunctionRef) {
            exists = C.allEntities.containsKey(((SILFunctionRef) TempFuncRef).getFunctionName());
        } else if (TempFuncRef instanceof SILFunctionRef.SILSummarizedFunctionRef) {
            exists = false;
        } else if (TempFuncRef instanceof SILFunctionRef.ArtificialFunctionRef) {
            exists = true;
        } else {
            Assertions.UNREACHABLE("FuncRef not in domain of correct types");
            exists = false;
        }
        CAstNode FuncNode;
        if (exists) {
            FuncNode = N.getChild(3);
        } else {
            if (N.getChildren().size() > 4) {
                FuncNode = N.getChild(4);
            } else {
                FuncNode = N.getChild(3);
            }
        }
        if (FuncNode.getKind() == CAstNode.UNARY_EXPR && !exists) {
        ...
        } else if (FuncNode.getKind() == CAstNode.BINARY_EXPR && !exists) {
            ...
            if (FuncRef instanceof SILFunctionRef) {
                ArrayList<CAstNode> Params = new ArrayList<>();
            ...
            } else if (FuncRef instanceof SILFunctionRef.ArtificialFunctionRef) {
                ArrayList<RawValue> generationParams = new ArrayList<>();
                for (CAstNode RawParam : FuncNode.getChildren()) {
                    String ParamName = (String) RawParam.getValue();
                    generationParams.add(new RawValue(ParamName, C.valueTable.getValue(ParamName).getType().getName()));
                }
                AbstractCodeEntity newEntity = generateNewEntity(FuncRef.getName(), generationParams, result);
                C.parent.addScopedEntity(null, newEntity);

                ArrayList<CAstNode> Params = new ArrayList<>();
                Params.add(((SILFunctionRef.ArtificialFunctionRef) FuncRef).getFunctionRef());
                Params.add(Ast.makeConstant("do"));
                for (CAstNode RawParam : FuncNode.getChildren()) {
                    String ParamName = (String) RawParam.getValue();
                    Params.add(C.valueTable.getValue(ParamName).getVarNode());
                ...
                Assertions.UNREACHABLE("Undefined operation");
    ...
    protected CAstNode visitBuiltin(CAstNode N, SILInstructionContext C) {
        RawValue result = getSingleResult(N);
        String BuiltinName = getStringValue(N, 2);
        ArrayList<RawValue> args = new ArrayList<>();
        ArrayList<CAstNode> params = new ArrayList<>();
        params.add(Ast.makeConstant(BuiltinName));
        params.add(Ast.makeConstant("do"));
        for (CAstNode argument : N.getChild(3).getChildren()) {
            String argumentName = (String) argument.getValue();
            RawValue argValue = new RawValue(argumentName, C.valueTable.getValue(argumentName).getType().getName());
            args.add(argValue);
            params.add(C.valueTable.getValue(argumentName).getVarNode());
        }
        AbstractCodeEntity newEntity = generateNewEntity(BuiltinName, args, result);
        CAstNode call = Ast.makeNode(CAstNode.CALL, params);
        C.parent.setGotoTarget(call, call);
        C.parent.addScopedEntity(null, newEntity);
        return call;
    }
    ...
    protected CAstNode visitBr(CAstNode N, SILInstructionContext C) {
        ...
        if (Inliner.shouldInlineBlock(DestBlockNo, C)) {
            ...
            return Inliner.doBlockInline(N, DestBlockNo, C, args, this);
        } else {
            for (CAstNode arg : N.getChild(1).getChildren()) {
    ...
    protected CAstNode visitSwitchEnum(CAstNode N, SILInstructionContext C) {
        // Convert switch to if*, goto (fallthrough).
        ...
        ArrayList<CAstNode> nodes = new ArrayList<>();
        for (CAstNode Case : N.getChild(1).getChildren()) {
            ...
            ArrayList<CAstNode> stmts = new ArrayList<>();
            if (Case.getChildren().size() > 2) {
                String ArgName = (String) Case.getChild(2).getValue();
                String ArgType = (String) Case.getChild(3).getValue();
                CAstNode Assign = EnumValue.copy(ArgName, ArgType);
                stmts.add(Assign);
            }
            if (Inliner.shouldInlineBlock(Integer.parseInt(DestBB), C)) {
                // Arguments already handled.
                stmts.add(Inliner.doBlockInline(N, Integer.parseInt(DestBB), C, new ArrayList<>(), this));
            } else {
                CAstNode GOTO = Ast.makeNode(CAstNode.GOTO, Ast.makeConstant(DestBB));
                tryGOTO(GOTO, DestBB, C);
                stmts.add(GOTO);
            }
            nodes.add(
                    Ast.makeNode(
                            ...
                            Ast.makeNode(
                                    ...
                                    CAstOperator.OP_EQ,
                                    Ast.makeNode(CAstNode.TYPE_OF, EnumValue.getVarNode()),
                                    Ast.makeConstant(CaseName)),
                            Ast.makeNode(CAstNode.BLOCK_STMT, stmts)
                    )
            );
        }
        // Default case fall through.
        if (N.getChildren().size() > 2) {
            CAstNode Case = N.getChild(2);
            String DestBB = (String) Case.getChild(0).getValue();
            ArrayList<CAstNode> stmts = new ArrayList<>();
            if (Case.getChildren().size() > 1) {
                String ArgName = (String) Case.getChild(1).getValue();
                String ArgType = (String) Case.getChild(2).getValue();
                CAstNode Assign = EnumValue.copy(ArgName, ArgType);
                stmts.add(Assign);
            }
            CAstNode GOTO = Ast.makeNode(CAstNode.GOTO, Ast.makeConstant(DestBB));
            tryGOTO(GOTO, DestBB, C);
            stmts.add(GOTO);
            nodes.add(Ast.makeNode(CAstNode.BLOCK_STMT, stmts));
        }
        nodes.add(0, Ast.makeNode(CAstNode.LABEL_STMT, Ast.makeConstant("SWITCH")));
    ...
    protected CAstNode visitTryApply(CAstNode N, SILInstructionContext C) {
            ...
            default: {
                ...
                if (FuncRef instanceof SILConstant) {
                    ...
                    Source = BuiltInFunctionSummaries.findSummary(
                            ((SILFunctionRef.SILSummarizedFunctionRef) FuncRef).getFunctionName(),
                            result.Name, result.Type, C, Params);
                    // If we return here, we skip the try/catch. This is probably fine.
                    if (Source == null || Source.getKind() == CAstNode.EMPTY) {
                        return Ast.makeNode(CAstNode.EMPTY);
                    } else if (Source.getKind() == CAstNode.VAR) {
                        C.valueTable.addValue(new SILValue(result.Name, result.Type, C));
        ...
        return Ast.makeNode(CAstNode.TRY,
                ...
                Ast.makeNode(CAstNode.CATCH,
                        Ast.makeConstant(errorReceiver.Name),
                        GotoError));
    }
}