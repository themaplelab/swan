...
import ca.ualberta.maple.swan.ir.Exceptions.{IncorrectSWANIRStructureException, UnexpectedSILFormatException, UnexpectedSILTypeBehaviourException}
import ca.ualberta.maple.swan.ir.{Argument, Block, BlockRef, Context, Function, FunctionAttribute, InstructionDef, Module, OperatorDef, Position, RefTable, SymbolRef, TerminatorDef}
...
/*
 * This is does not include the translation of each instruction
...
trait ISILToRawSWANIR {
  ...
  protected val missingStructs = mutable.Set[String]()

  @throws[IncorrectSWANIRStructureException]
  ...
  def translateSILModule(silModule: SILModule): Module = {
    ...
    silModule.functions.foreach((silFunction: SILFunction) => {
      // TODO: Passing stuff down like this is dumb
      functions.append(translateSILFunction(silModule, silFunction))
    })
    new Module(functions, silModule.imports)
  }
  ...
  private def translateSILFunction(silModule: SILModule, silFunction: SILFunction): Function = {
    ...
    silFunction.blocks.foreach((silBlock: SILBlock) => {
      blocks.append(translateSILBlock(silModule, silFunction, silBlock, refTable))
    })
    val coroutine = if (isCoroutine(silFunction)) Some(FunctionAttribute.coroutine) else None
    new Function(coroutine, silFunction.name.demangled, Utils.SILTypeToType(silFunction.tpe), blocks, refTable)
  }

  @throws[IncorrectSWANIRStructureException]
  @throws[UnexpectedSILFormatException]
  @throws[UnexpectedSILTypeBehaviourException]
  private def translateSILBlock(silModule: SILModule, silFunction: SILFunction,
    ...
    silBlock.operatorDefs.foreach((silOperatorDef: SILOperatorDef) => {
      breakable {
        ...
        val ctx = new Context(silModule, silFunction, silBlock, position, refTable)
        ...
        instructions.foreach((inst: InstructionDef) => {
          if (!inst.isInstanceOf[InstructionDef.operator]) {
            throw new IncorrectSWANIRStructureException("Operator expected")
          }
        ...
        instructions.zipWithIndex.foreach(term => {
          ...
          if (term._2 != instructions.length - 1) {
            if (!instruction.isInstanceOf[InstructionDef.operator]) {
              throw new IncorrectSWANIRStructureException("All instructions before the last instruction must be operators")
            }
            ...
            if (!instruction.isInstanceOf[InstructionDef.terminator]) {
              throw new IncorrectSWANIRStructureException("Last instruction must be a terminator.")
            }
            terminator = instruction.asInstanceOf[InstructionDef.terminator].terminatorDef
          }
        })
        if (terminator == null) {
          throw new IncorrectSWANIRStructureException("Terminator expected for block " +
